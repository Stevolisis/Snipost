...
import {  commentsFailure, loadCommentsStart, updateCommentAfterVote } from '@/lib/redux/slices/comments';
...
const ReplyBox = ({comment, setShowReplyBox}) => {
    ...
    const insertTextAtCursor = (prefix, suffix) => {
        if (!textareaRef.current) return { text: reply, cursorPos: cursorPosition };
        
        const startPos = textareaRef.current.selectionStart;
        const endPos = textareaRef.current.selectionEnd;
        const selectedText = reply.substring(startPos, endPos);
        
        const before = reply.substring(0, startPos);
        const after = reply.substring(endPos);
    ...
    const selectUser = (user) => {
        const before = reply.substring(0, mentionStartPos - 1); // Remove the % part
        const after = reply.substring(cursorPosition);
            ...
            <Tabs defaultValue="write" className="">
                <div className="flex justify-between items-center w-full">
                    ...
                    <div className='flex items-center gap-x-2'>
                        ...
                        <Popover open={showMentionsDropdown} onOpenChange={setShowMentionsDropdown}>
                ...
                <TabsContent value="preview">
                    <div className="min-h-32">
                        {reply ? (
                            <div>{renderText(reply,mentions)}</div>
                        ) : (
